...
export interface ITicketContext {
  ...
  setNewTransferStops: (
    newTransferStops: TTransferStops,
    only?: boolean,
  ) => void;
}
...
export const TicketContext =
  createContext<ITicketContext>(initialTicketContext);
export const TicketProvider = ({ children }: ITicketProviderProps) => {
  ...
  const setNewTransferStops = (
    ...
    if (index === -1) {
      setTransferStops((prevState) => [
        ...prevState.filter((stops) => stops !== STOPS_ALL),
        newTransferStops,
      ]);
    } else {
